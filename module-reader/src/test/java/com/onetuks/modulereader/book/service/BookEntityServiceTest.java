...
class BookEntityServiceTest extends ReaderIntegrationTest {
  ...
  void readBookTest() {
    // Given
    int randomIndex = new Random().nextInt(bookEntities.size());
    long bookId = bookEntities.get(randomIndex).getBookId();
    BookEntity expected = bookEntities.get(randomIndex);
    long expectedViewCount = expected.getBookStaticsEntity().getViewCount();
    ...
    // Then
    assertAll(
        () -> assertThat(result.authorId()).isEqualTo(expected.getAuthorEntity().getAuthorId()),
        ...
        () ->
            assertThat(result.favoriteCount())
                .isEqualTo(expected.getBookStaticsEntity().getFavoriteCount()),
        () -> assertThat(result.viewCount()).isEqualTo(expectedViewCount + 1),
        () -> assertThat(result.salesCount()).isEqualTo(expected.getBookStaticsEntity().getSalesCount()),
        () ->
            assertThat(result.commentCount())
                .isEqualTo(expected.getBookStaticsEntity().getCommentCount()));
  }
  ...
  void readBooksTest() {
    // Given
    int index = new Random().nextInt(bookEntities.size());
    BookEntity soldBookEntity = bookEntities.get(index);

    String title = soldBookEntity.getBookConceptualInfo().getTitle();
    String authorNickname = soldBookEntity.getAuthorNickname();
    Category category =
        soldBookEntity.getBookConceptualInfo().getCategories().stream().findAny().orElse(Category.NOVEL);
