...
class MemberEntityServiceTest extends ReaderIntegrationTest {
  ...
  @BeforeEach
  void setUp() {
    savedMemberEntity = memberJpaRepository.save(MemberFixture.create(RoleType.USER));
  }
  ...
  void updateMemberInfo_Test() {
    ...
    // When
    MemberEntryInfoResult result = memberService.updateMemberInfo(savedMemberEntity.getMemberId(), param);

    // Then
    assertAll(
        () -> assertThat(result.memberId()).isEqualTo(savedMemberEntity.getMemberId()),
  ...
  void updateMemberInfo_DuplicatedNickname_ExceptionTest() {
    // Given
    MemberEntity otherMemberEntity = memberJpaRepository.save(MemberFixture.create(RoleType.USER));
    MemberEntryInfoParam param = new MemberEntryInfoParam(savedMemberEntity.getNickname(), true);

    // When & Then
    assertThrows(
        DataIntegrityViolationException.class,
        () -> {
          memberService.updateMemberInfo(otherMemberEntity.getMemberId(), param);
  ...
  void updateMemberProfileTest() {
    ...
    FileWrapper fileWrapper =
        FileWrapperFixture.createFile(savedMemberEntity.getMemberId(), FileType.PROFILES);

    // When
    MemberProfileEditResult result =
        memberService.updateMemberProfile(savedMemberEntity.getMemberId(), param, fileWrapper);
  ...
  void updateDetaultAddressTest() {
    ...
    MemberDefaultAddressEditResult result =
        memberService.updateDetaultAddress(savedMemberEntity.getMemberId(), param);
  ...
  void changeDefaultCashReceiptInfoTest() {
    ...
    MemberDefaultCashReceiptEditResult result =
        memberService.updateDefaultCashReceipt(savedMemberEntity.getMemberId(), param);
  ...
  void readMemberInfoTest() {
    // Given & When
    MemberInfoResult result = memberService.readMemberInfo(savedMemberEntity.getMemberId());

    // Then
    assertAll(
        () -> assertThat(result.memberId()).isEqualTo(savedMemberEntity.getMemberId()),
        () -> assertThat(result.nickname()).isEqualTo(savedMemberEntity.getNickname()),
        () -> assertThat(result.profileImgUrl()).isEqualTo(savedMemberEntity.getProfileImgUrl()),
        () -> assertThat(result.alarmPermission()).isEqualTo(savedMemberEntity.getAlarmPermission()),
        () -> assertThat(result.defaultAddress()).isEqualTo(savedMemberEntity.getDefaultAddress()),
        () ->
            assertThat(result.defaultAddressDetail())
                .isEqualTo(savedMemberEntity.getDefaultAddressDetail()),
        () ->
            assertThat(result.defaultCashReceiptType())
                .isEqualTo(savedMemberEntity.getDefaultCashReceiptType()),
        () ->
            assertThat(result.defaultCashReceiptNumber())
                .isEqualTo(savedMemberEntity.getDefaultCashReceiptNumber()));
  }
