...
import com.onetuks.dbstorage.book.repository.BookJpaRepository;
...
import com.onetuks.dbstorage.fixture.RegistrationFixture;
...
import com.onetuks.dbstorage.registration.repository.RegistrationJpaRepository;
import com.onetuks.filestorage.fixture.FileWrapperFixture;
import com.onetuks.filestorage.repository.S3Repository;
import com.onetuks.filestorage.vo.FileType;
import com.onetuks.modulescm.ScmDomainIntegrationTest;
...
class RegistrationEntityScmServiceTest extends ScmDomainIntegrationTest {
  ...
  void createRegistrationTest() {
    ...
    List<File> savedPreviewFiles =
        previewFiles.fileWrappers().stream().map(file -> s3Repository.getFile(file.getUri()))
            .toList();
  ...
  void createRegistration_NoCoverImgFile_ExceptionTest() {
    ...
    FileWrapperCollection previewFiles =
        FileWrapperFixture.createFiles(authorId, FileType.PREVIEWS);
    FileWrapper sampleFile = FileWrapperFixture.createFile(authorEntity.getAuthorId(),
        FileType.SAMPLES);
  ...
  void createRegistration_NoSampleFile_ExceptionTest() {
    // Given
    long authorId = authorEntity.getAuthorId();
    FileWrapper coverImgFile = FileWrapperFixture.createFile(authorEntity.getAuthorId(),
        FileType.COVERS);
    FileWrapperCollection detailImgFiles =
        FileWrapperFixture.createFiles(authorId, FileType.DETAILS);
    FileWrapperCollection previewFiles =
  ...
  void updateRegistrationApprovalInfoTest() {
    // Given
    RegistrationEntity save = registrationJpaRepository.save(
        RegistrationFixture.create(authorEntity));
  ...
  void readRegistrationTest() {
    ...
    // When
    RegistrationResult result =
        registrationScmService.readRegistration(authorEntity.getAuthorId(),
            save.getRegistrationId());
