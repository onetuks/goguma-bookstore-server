...
class BookEntityScmServiceTest extends ScmIntegrationTest {
  ...
  void updateBookTest() {
    ...
    // Then
    assertAll(
        () -> assertThat(result.bookId()).isEqualTo(bookEntity.getBookId()),
        ...
        () -> assertThat(result.title()).isEqualTo(bookEntity.getTitle()),
        ...
        () -> assertThat(result.isbn()).isEqualTo(bookEntity.getIsbn()),
        () -> assertThat(result.height()).isEqualTo(bookEntity.getHeight()),
        () -> assertThat(result.width()).isEqualTo(bookEntity.getWidth()),
        () -> assertThat(result.coverType()).isEqualTo(bookEntity.getCoverType()),
        () -> assertThat(result.pageCount()).isEqualTo(bookEntity.getPageCount()),
        ...
        () -> assertThat(result.publisher()).isEqualTo(bookEntity.getPublisher()),
  ...
  void updateBook_NoFiles_Test() {
    ...
    assertAll(
        ...
        () -> assertThat(result.publisher()).isEqualTo(bookEntity.getPublisher()),
        () -> assertThat(result.stockCount()).isEqualTo(editParam.stockCount()),
        () -> assertThat(result.coverImgUrl()).isEqualTo(bookEntity.getCoverImgUrl()),
        () ->
            assertThat(result.detailImgUrls())
                .containsExactlyInAnyOrderElementsOf(
                    detailImgFiles.getUris().stream().map(uri -> BASE_URL + uri).toList()),
  ...
  void getAllBooksByAuthorTest() {
    // Given
    long loginAuthorId = authorEntity.getAuthorId();
    long authorId = authorEntity.getAuthorId();
    ...
    assertThat(results)
        ...
        .allSatisfy(
            result -> {
              ...
              assertThat(result.authorNickname()).isEqualTo(authorEntity.getNickname());
              assertThat(result.title()).isEqualTo(bookEntity.getTitle());
              assertThat(result.oneLiner()).isEqualTo(bookEntity.getOneLiner());
              assertThat(result.summary()).isEqualTo(bookEntity.getSummary());
              assertThat(result.categories())
                  .containsExactlyInAnyOrderElementsOf(bookEntity.getCategories());
              assertThat(result.isbn()).isEqualTo(bookEntity.getIsbn());
              assertThat(result.height()).isEqualTo(bookEntity.getHeight());
              assertThat(result.width()).isEqualTo(bookEntity.getWidth());
              assertThat(result.coverType()).isEqualTo(bookEntity.getCoverType());
              assertThat(result.pageCount()).isEqualTo(bookEntity.getPageCount());
              assertThat(result.regularPrice()).isEqualTo(bookEntity.getRegularPrice());
              assertThat(result.purchasePrice()).isEqualTo(bookEntity.getPurchasePrice());
              assertThat(result.promotion()).isEqualTo(bookEntity.isPromotion());
              assertThat(result.publisher()).isEqualTo(bookEntity.getPublisher());
              assertThat(result.stockCount()).isEqualTo(bookEntity.getStockCount());
              assertThat(result.coverImgUrl()).isEqualTo(bookEntity.getCoverImgUrl());
              assertThat(result.detailImgUrls())
                  .containsExactlyInAnyOrderElementsOf(bookEntity.getDetailImgUrls());
              assertThat(result.previewUrls())
                  .containsExactlyInAnyOrderElementsOf(bookEntity.getPreviewUrls());
            });
  }
